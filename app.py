...
def synth_tone_wav(hz: float, seconds: float = 10.0, sr: int = 44100, amp: float = 0.2) -> bytes:
    ...
    # fade in/out de 20 ms para evitar 'click'
    ramp = int(sr * 0.02)
    if ramp > 0:
...
with tabs[4]:
    ...
    if not df.empty:
        ...
        st.markdown("### 🔊 Ouvir uma frequência")

        # Monta opções legíveis: "CODE • Nome — 528.0 Hz"
        def _label(row):
            code = str(row.get("code") or "").strip()
            nome = str(row.get("nome") or "").strip()
            hz   = row.get("hz")
            hz_s = f"{float(hz):.2f} Hz" if pd.notnull(hz) else "—"
            base = f"{code}" if code else "(sem code)"
            if nome:
                base += f" • {nome}"
            return f"{base} — {hz_s}"

        # Opções e mapa
        opts = df.apply(_label, axis=1).tolist()
        mapa_opt_idx = {opts[i]: i for i in range(len(opts))}
        sel_label = st.selectbox("Selecione a frequência", opts, key=K("freq","ouvir","sel"))
        row = df.iloc[mapa_opt_idx[sel_label]]
        hz_sel = float(row.get("hz") or 0.0)

        col_top1, col_top2, col_top3 = st.columns([1,1,1])
        col_top1.metric("Frequência (Hz)", f"{hz_sel:.2f}")
        col_top2.metric("Chakra", (row.get("chakra") or "—").title() if isinstance(row.get("chakra"), str) else "—")
        col_top3.metric("Tipo", row.get("tipo") or "—")

        # Modo de reprodução
        modo = st.radio("Modo", ["Tom puro", "Binaural (diferença)"], horizontal=True, key=K("freq","ouvir","modo"))
        dur  = st.slider("Duração (s)", 5, 120, 20, 5, key=K("freq","ouvir","dur"))

        if modo == "Tom puro":
            # Player WebAudio (leve) + opção de baixar um WAV curto
            st.components.v1.html(
                webaudio_tone_html(hz_sel, seconds=dur, gain=0.06, wave="sine"),
                height=160
            )
            ...
            st.download_button(
                ...
                mime="audio/wav",
                key=K("freq","ouvir","dl_tone")
            )
        else:
            # Binaural centrado na frequência selecionada
            beat = st.slider("Batida (Hz)", 0.5, 45.0, 10.0, 0.5, key=K("freq","ouvir","beat"))
            bt = abs(float(beat))
