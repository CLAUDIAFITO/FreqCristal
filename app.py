...
# ========== Sessões (HISTÓRICO + EXECUÇÃO) ==========
with tabs[12]:
    st.subheader("Sessões — histórico e execução")
    if not sb:
        st.warning("Configure SUPABASE_URL/KEY para listar as sessões.")
    else:
        # --- filtros ---
        pts = sb_select("patients","id,nome",order="created_at",desc=True)
        mapa_pac = {p["nome"]: p["id"] for p in (pts or [])}
        pac_opt = ["(Todos)"] + list(mapa_pac.keys())
        c1, c2 = st.columns([2,1])
        pac_sel = c1.selectbox("Paciente (opcional)", pac_opt, key=K("sess_page","pac"))
        pid = None if pac_sel == "(Todos)" else mapa_pac.get(pac_sel)
        status_sel = c2.selectbox("Status", ["todos","rascunho","finalizada"], index=0, key=K("sess_page","status"))

        # --- catálogo de frequências (para tocar) ---
        freq_rows = sb_select("frequencies","code,nome,hz",order="code")
        code_to_label = { str(f.get("code")): f'{f.get("code")} • {f.get("nome","")}' for f in (freq_rows or []) }
        code_to_hz    = { str(f.get("code")): float(f.get("hz") or 0.0) for f in (freq_rows or []) }

        # --- busca sessões ---
        try:
            q = sb.table("sessions").select("id,patient_id,data,tipo,protocolo,status,patients(nome)").order("data", desc=True).limit(200)
            if pid is not None: q = q.eq("patient_id", pid)
            if status_sel != "todos": q = q.eq("status", status_sel)
            sess_rows = q.execute().data or []
        except Exception:
            sess_rows = []

        if not sess_rows:
            st.info("Nenhuma sessão encontrada com os filtros atuais.")
        else:
            import pandas as _pd
            ...
            def _proto_summary(p: dict):
                ...
                nfreq = len(fqs)
                bname = (p.get("binaural") or {}).get("nome") or "—"
                cname = (p.get("cama") or {}).get("nome") or "—"
                pname = (p.get("fitoterapia_plan") or {}).get("name") or "—"
                return (bname, cname, pname, nfreq)
            ...
            for r in sess_rows:
                ...
                table.append({
                    "Quando": data_str,
                    "Paciente": (r.get("patients") or {}).get("nome","—") if isinstance(r.get("patients"), dict) else "—",
                    "Tipo": r.get("tipo") or "—",
                    "Status": r.get("status") or "—",
                    "Binaural": b,
                    "Cama": c,
            ...
            df_sess = _pd.DataFrame(table)
            st.dataframe(
                df_sess[["Quando","Paciente","Tipo","Status","Binaural","Cama","Plano Fito","Frequências (#)"]],
                use_container_width=True, hide_index=True
            )
            ...
            for r in sess_rows:
                ...
                with st.expander(f"🧭 Sessão #{sid} — {r.get('tipo','Misto')}  ·  {r.get('status','rascunho')}"):
                    ...
                    codes = [ (fx.get("code") if isinstance(fx, dict) else str(fx)) for fx in fqs ]
                    if codes:
                        labels = [ code_to_label.get(str(c), str(c)) for c in codes ]
                        sel_fx = st.selectbox("Tocar frequência do protocolo", labels, key=K("sess_page","fx",sid))
                        code_sel = str(sel_fx).split(" • ")[0]
                        hz_sel = code_to_hz.get(code_sel)
                        if hz_sel:
                            st.components.v1.html(webaudio_tone_html(hz_sel, seconds=20), height=160)
                        else:
                            st.caption("Sem Hz no catálogo — cadastre em **Frequências** para tocar aqui.")
                    else:
                        st.caption("Frequências: —")
                    ...
                    st.download_button(
                        "⬇️ Exportar protocolo (JSON)",
                        data=json.dumps(prot, ensure_ascii=False, indent=2),
                        file_name=f"protocolo_sessao_{sid}.json",
                        mime="application/json",
                        key=K("sess_page","export",sid)
                    )

                    st.markdown("---")

                    # --- Aplicar no Planner (pré-preenche) ---
                    if st.button("📋 Aplicar no Planner (pré-preencher)", key=K("sess_page","apply",sid)):
                        try:
                            # Frequências -> multiselect do Planner
                            labels = [ code_to_label.get(str(c.get("code") if isinstance(c, dict) else c),
                                                         str(c.get("code") if isinstance(c, dict) else c))
                            ...
                            # Binaural / Cama / Fitoplan por nome
                            bname = (prot.get("binaural") or {}).get("nome")
                            cname = (prot.get("cama") or {}).get("nome")
                            pname = (prot.get("fitoterapia_plan") or {}).get("name")
                            if bname: st.session_state[K("planner","binaural")] = bname
                            if cname: st.session_state[K("planner","cama")]     = cname
                            if pname: st.session_state[K("planner","fitoplan")] = pname

                            st.session_state[K("planner","notas")] = prot.get("notas","")
                            st.success("Protocolo aplicado ao Planner. Abra a aba **Sessão (Planner)** para editar/executar.")
                            st.rerun()
                        except Exception as e:
                            st.error(f"Falha ao aplicar: {e}")
                    ...
                    colF, colD, colX = st.columns(3)
                    # --- Finalizar ---
                    if colF.button("✅ Finalizar sessão", key=K("sess_page","close",sid)) and sb:
                        try:
                            sb.table("sessions").update({"status":"finalizada"}).eq("id", sid).execute()
                            st.success("Sessão finalizada."); st.cache_data.clear(); st.rerun()
                        except Exception as e:
                            st.error(f"Erro ao finalizar: {getattr(e,'message',e)}")

                    # --- Duplicar ---
                    if colD.button("🧬 Duplicar como rascunho", key=K("sess_page","dup",sid)) and sb:
                        try:
                            sb.table("sessions").insert({
                                "patient_id": r.get("patient_id"),
                                "data": datetime.utcnow().isoformat(),
                            ...
                            st.success("Sessão duplicada."); st.cache_data.clear(); st.rerun()
                        except Exception as e:
                            st.error(f"Erro ao duplicar: {getattr(e,'message',e)}")

                    # --- Excluir ---
                    del_ok = colX.checkbox("Confirmar excluir", key=K("sess_page","confirmdel",sid))
                    if colX.button("🗑️ Excluir sessão", key=K("sess_page","del",sid)) and sb:
                        if not del_ok:
                            st.warning("Marque 'Confirmar excluir' antes.")
                        else:
                            try:
                                sb.table("sessions").delete().eq("id", sid).execute()
                                st.success("Sessão excluída."); st.cache_data.clear(); st.rerun()
                            except Exception as e:
                                st.error(f"Erro ao excluir: {getattr(e,'message',e)}")
